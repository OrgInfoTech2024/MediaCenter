...
class MediaCenter(QWidget):
    ...
    def openFile(self, fileName):
        ...
        if fileName.lower().endswith(('.png', '.bmp', '.jpg', '.jpeg', '.gif', '.ico', '.svg', '.svgz' '.webp')):
            ...
            self.top_overlay.setGeometry(0, 0, self.display_width, 55)
            self.top_overlay.setStyleSheet("background-color: #003153;")
            self.top_overlay.show()

            self.bottom_overlay = QWidget(self)
            self.bottom_overlay.setGeometry(0, self.display_height - 100, self.display_width, 100)
            self.bottom_overlay.setStyleSheet("background-color: #003153;")
            ...
            self.timeLabel.raise_()
            self.lastMediaButton.raise_()
            self.nextMediaButton.raise_()
